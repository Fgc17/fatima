...
pub(crate) fn encrypt_json<T: Serialize>(value: &T, key: &[u8]) -> Result<EncryptedBlob> {
    let bytes =
        serde_json::to_vec(value).map_err(|error| FatimaError::message(error.to_string()))?;
    encrypt_bytes(&bytes, key)
}
...
pub(crate) fn decrypt_json<T: for<'de> Deserialize<'de>>(
    ...
    serde_json::from_slice(&bytes).map_err(|error| FatimaError::message(error.to_string()))
}
