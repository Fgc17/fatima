...
use fatima_core::ProviderConfig;
use serde_json::json;
...
fn file_provider_loads_env_file() {
    ...
    let env = load_provider(temp.path(), "development", &provider, &BTreeMap::new())
...
fn file_provider_rejects_unsupported_format() {
    ...
    let error = load_provider(temp.path(), "development", &provider, &BTreeMap::new())
