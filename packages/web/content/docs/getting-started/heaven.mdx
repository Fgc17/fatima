...
To enable runtime reloading, we will use `@fatimajs/tools/heaven` with the `watch` method.
...
export async function register() {
  if (process.env.NEXT_RUNTIME === "nodejs") {
    const { heaven } = await import("@fatimajs/tools/heaven");
...
import { heaven } from "@fatimajs/tools/heaven";
