...
use crate::vault::controllers::{close_modal, copy_generated_user_key, submit_modal};
...
pub fn handle_key(
    ...
    if state.modal == Some(Modal::UserKey) && state.generated_user_key.is_some() {
        return handle_generated_user_key_key(state, event);
    }
    ...
    match event.code {
        ...
        KeyCode::Char(' ')
            if state.modal == Some(Modal::UserKey)
                && state.modal_focus == ModalFocus::EnvironmentList =>
        {
            toggle_user_key_environment(state)
        }
...
fn handle_generated_user_key_key(state: &mut VaultAppState, event: KeyEvent) -> Result<()> {
    match event.code {
        KeyCode::Esc => close_modal(state),
        KeyCode::Enter | KeyCode::Char('c') | KeyCode::Char('C') => {
            if let Err(error) = copy_generated_user_key(state) {
...
fn next_modal_focus(state: &mut VaultAppState) {
    if state.modal == Some(Modal::UserKey) && state.modal_focus == ModalFocus::EnvironmentList {
        state.user_key_environment_cursor = (state.user_key_environment_cursor + 1)
    ...
    state.modal_focus = match state.modal {
        ...
        Some(Modal::UserKey) => match state.modal_focus {
...
fn previous_modal_focus(state: &mut VaultAppState) {
    if state.modal == Some(Modal::UserKey) && state.modal_focus == ModalFocus::EnvironmentList {
        if state.user_key_environment_cursor > 0 {
            state.user_key_environment_cursor = state.user_key_environment_cursor.saturating_sub(1);
        } else {
...
fn toggle_user_key_environment(state: &mut VaultAppState) {
    let Some(environment) = state
        .environment_list()
        .get(state.user_key_environment_cursor)
        .cloned()
    else {
        return;
    };
    if let Some(index) = state
        .user_key_environments
        .iter()
        .position(|value| value == &environment)
    {
        state.user_key_environments.remove(index);
    } else {
        state.user_key_environments.push(environment);
    }
}
